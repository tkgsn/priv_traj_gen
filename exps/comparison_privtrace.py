...
def command(epsilon, divide_parameter):
    return f'docker run --rm -v /mnt/data:/data -e DATASET={dataset} -e MAX_SIZE={max_size} -e TOTAL_EPSILON={epsilon} -e FIXED_DIVIDE_PARAMETER={divide_parameter} kyotohiemrnet.azurecr.io/hiemrnet_cu117 /bin/bash -c "cd ./competitors/privtrace && ./run.sh"' ,\
            f'docker run --rm -v /mnt/data:/data -e TEST_THRESH=30 -e ABLATION=False -e SEED=0 -e TRUNCATE=0 -e MODEL_DIR={data_dir / f"privtrace_{divide_parameter}" / f"model_{epsilon}"} -e EVAL_INTERVAL=10 -e EVAL_DATA_DIR={data_dir} kyotohiemrnet.azurecr.io/hiemrnet_cu117 /bin/bash -c "./evaluate.sh"'

with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
    for fixed_divide_parameter in fixed_divide_parameters:
        for epsilon in epsilons:
            command1, command2 = command(epsilon, fixed_divide_parameter)
