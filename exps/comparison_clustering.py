...
def command(epsilon, k):
    return f'docker run --rm -v /mnt/data:/data -e DATASET={dataset} -e MAX_SIZE={max_size} -e EPSILON={epsilon} -e K={k} -e N_BINS={n_bins} kyotohiemrnet.azurecr.io/hiemrnet_cu117 /bin/bash -c "cd ./competitors/clustering && ./run.sh"' ,\
            f'docker run --rm -v /mnt/data:/data -e TEST_THRESH=30 -e ABLATION=False -e SEED=0 -e TRUNCATE=0 -e MODEL_DIR={data_dir / f"clustering_{k}" / f"model_{epsilon}"} -e EVAL_INTERVAL=1 -e EVAL_DATA_DIR={data_dir} kyotohiemrnet.azurecr.io/hiemrnet_cu117 /bin/bash -c "./evaluate.sh"'
