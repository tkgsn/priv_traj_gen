...
class Generator(nn.Module):
    ...
    def make_sample(self, references, start_idx, batch_size):
        '''
        ...
        for i in range(int(n_samples / batch_size)):
            ...
            # make initial input
            locations = torch.tensor([[start_idx, reference[i][0]] for i in range(batch_size)]).to(next(self.parameters()).device).view(batch_size, 2)
            times = torch.tensor([[0,0] for _ in range(batch_size)]).to(next(self.parameters()).device).view(batch_size, 2)
            ...
            for _ in range(seq_len):
                ...
                prefix_embedding = hiddens[:, -1, :].detach()
...
def construct_baseline_generator(n_locations, n_times, location_embedding_dim, time_embedding_dim, hidden_dim):
    ...
    time_encoding_component = MatrixTimeEncodingComponent(n_times, time_embedding_dim)
    prefix_encoding_component = GRUPrefixEncodingComponent(hidden_dim, 1, False)
    scoring_component = LinearScoringComponent(hidden_dim, n_locations, n_times)
    return Generator(location_encoding_component, time_encoding_component, prefix_encoding_component, scoring_component)
...
class BaseReferenceGenerator(nn.Module):
    ...
    def make_initial_input(self, reference):
        batch_size = len(reference)
        seq_len = max([len(ref) for ref in reference])
        outputs = torch.tensor([[self.start_idx, reference[i][0]] for i in range(batch_size)]).view(batch_size, 1)
...
def construct_generator(model_name, n_locations, n_times, location_embedding_dim, time_embedding_dim, memory_hidden_dim, multitask):
    if model_name == "baseline":
        ...
        location_encoding_component = LinearHierarchicalLocationEncodingComponent(TrajectoryDataset.vocab_size(n_locations), location_embedding_dim)
        time_encoding_component = MatrixTimeEncodingComponent(n_times-1, time_embedding_dim)
        input_dim = location_embedding_dim + time_embedding_dim
        prefix_encoding_component = GRUPrefixEncodingComponent(input_dim, memory_hidden_dim, 1, False)
